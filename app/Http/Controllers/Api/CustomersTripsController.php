...
class CustomersTripsController extends Controller
{
    ...
    public function formatTripDetailsWithKey($row)
    {
        ...
        $newdata['trip']['rating'] =  isset($row->customer_id) ? $this->userRating($row->customer_id)  : '';
    ...
    public function my_current_trip(Request $request)
    {
        ...
        $query      = Trip::with('car', 'service', 'paymentMethod', 'driver', 'customer')->where('payment_status',0)->where($user_field, $user_id)->whereIn('status', $status)->orderBy('updated_at', 'desc'); //status 1-8
