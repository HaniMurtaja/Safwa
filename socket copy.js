...
io.on("connection", (socket) => {
    ...
    socket.on('chat', function(object) {  // {to,msg}
        ...
        if (socket.auth=="yes"){
            ...
            var status=1;
            if (socket_id_to!='') status=2;
            object.status=status;
            var created_at=new Date().toISOString().slice(0, 19).replace('T', ' ');
            object.date=created_at;
            let msg_id=1;
            connection.query("insert into messages(sender_id,reply_to_id,message_type,message,created_at,status) VALUES(?,?,?,?,?,?) ", [object.from,object.to,1,object.msg,object.date,object.status], function (err, result, fields) {
                      msg_id=result.insertId;
            });

            console.log("b"+msg_id);
            object.id=msg_id;
