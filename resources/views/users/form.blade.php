...
<div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
    <label for="email" class="col-md-2 control-label">{{ trans('users.permission') }}</label>
    <div class="col-md-10">
        <select class="form-control" id="user_type_id" name="user_type_id" required="true">
                <option value="" style="display: none;" {{ old('user_type_id', optional($user)->user_type_id ?: '') == '' ? 'selected' : '' }} disabled selected>{{ trans('users.country__placeholder') }}</option>
            @foreach ($permissions as $key => $permission)
                <option value="{{ $permission->id }}" {{ old('user_type_id', optional($user)->user_type_id) == $key ? 'selected' : '' }}>
                    {{ $permission->user_type }}
                </option>
            @endforeach
        </select>

        {!! $errors->first('user_type_id', '<p class="text-danger">:message</p>') !!}
    </div>
</div>
